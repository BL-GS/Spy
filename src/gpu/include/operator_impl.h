...
namespace spy::gpu {
	...
	public: /* Interface for schedule and execution */
		 ...
		 */
		static OperatorStatus execute([[maybe_unused]] GPUBackend *backend_ptr, [[maybe_unused]] const OperatorEnvParam &param, [[maybe_unused]] OperatorNode *op_node) { return OperatorStatus::Unsupport; }
	};
...
#define OperatorDefinition(op_type)                                 \
    ...
    struct OperatorImpl<OperatorType:: op_type> {                   \
        static size_t get_task_num(const GPUBackend *backend_ptr, const OperatorNode *op_node) {            \
            return Operator##op_type##Impl::get_task_num(backend_ptr, op_node);                             \
        }                                                                                                   \
                                                                                                            \
        static size_t get_buffer_size(const GPUBackend *backend_ptr, const OperatorNode *op_node) {         \
            return Operator##op_type##Impl::get_buffer_size(backend_ptr, op_node);                          \
        }                                                                                                   \
                                                                                                            \
        static OperatorStatus execute(GPUBackend *backend_ptr, const OperatorEnvParam &param, OperatorNode *op_node) {              \
