...
namespace spy {
	...
	public:
		AbstractVocab(const GGUFContext &context):
	...
	protected: /* Initialization */
		void init(const GGUFContext &context) {
		...
		void init_token_table(const GGUFContext &context) {
			const auto &token = context.find_gguf_value(LLMKey::TOKENIZER_LIST).get_value<GGUFArray>();
			...
			if (score_option.has_value()) {
				const GGUFArray &score_array = score_option->get_value<GGUFArray>();
				for (int i = 0; i < num_vocab; ++i) {
			...
			if (token_type_option.has_value()) {
				const GGUFArray &type_array = token_type_option->get_value<GGUFArray>();
				for (int i = 0; i < num_vocab; ++i) {
		...
		void init_special_token(const GGUFContext &context) {
		...
		void init_eos_bos(const GGUFContext &context) {
		...
		void init_special_token_cache(const GGUFContext &context) {
		...
		virtual void init_newline_token(const GGUFContext &context) = 0;
