...
namespace spy {
	...
	public:
		static constexpr auto   ModelMeta_MAGIC 		= ModelMetaContext::ModelMeta_MAGIC;
		static constexpr size_t ModelMeta_MAGIC_SIZE = ModelMeta_MAGIC.size();

	public:
		static std::pair<std::unique_ptr<ModelMetaContext>, ModelType> init_from_file(const std::string_view filename) {
			auto      context_ptr = std::make_unique<ModelMetaContext>();
			...
			/* Read magic */
			try {
				read_gguf_array(file, context.header.magic.data(), ModelMeta_MAGIC_SIZE);
				spy_assert(context.header.magic == ModelMeta_MAGIC,
				           "Invalid magic characters (given: {}, expect: {})",
				           std::string_view(context.header.magic.data(), ModelMeta_MAGIC_SIZE), 
						   std::string_view(ModelMeta_MAGIC.data(), ModelMeta_MAGIC_SIZE));
