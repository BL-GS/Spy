...
namespace spy {
	...
	public:
		...
		Count &data_input(const DataNode *node_ptr)      { return data_input_array_[node_ptr->id]; }

		Count &data_output(const DataNode *node_ptr)     { return data_output_array_[node_ptr->id]; }

		Count &  op_input(const OperatorNode *node_ptr)  { return op_input_array_[node_ptr->id];   }
	};
